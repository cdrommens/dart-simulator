...
public class ThrowSimulator {
    ...
    public Throw throwDart(Turn turn, Dart dart, int currentScore, Player player) {
        ...
        if ((currentScore <= 40 && currentScore >= 2) && (currentScore % 2 == 0)) {
            ...
            return throwDart(turn, dart, DOUBLE, currentScore / 2, player, true);
        } else if (currentScore == 50) {
            //throw bull
            return throwDart(turn, dart, BULL, 50, player, true);
        } else {
            ...
            if (aim == 60 || aim == 57 || aim == 54 || aim == 51 || aim == 48 || aim == 45 || aim == 42 || aim == 39 || aim == 33 || aim == 30) {
                ...
                return throwDart(turn, dart, TREBLE, aim / 3, player, false);
            } else {
                ...
                if (aim > 20 && aim % 2 == 0) { //see last dart in optimal path (double should now be single)
                    return throwDart(turn, dart, SINGLE, aim / 2, player, false);
                } else {
                    return throwDart(turn, dart, SINGLE, aim, player, false);
                }
            }
        }
    }

    private Throw throwDart(Turn turn, Dart dart, String aimType, int scoreToAim, Player player, boolean isFinishingShot) {
        if (throwSimulationStrategies.containsKey(aimType)) {
            return throwSimulationStrategies.get(aimType).simulateThrow(turn, dart, scoreToAim, player, isFinishingShot);
        }
