...
//TODO : introduce levels : lower the aimed hit rate with a certain %
...
public class Board {
    ...
    public SingleScore throwBull(int accuracyPercentage) {
    ...
    public SingleScore throwTreble(int treble, int accuracyPercentage) {
        ...
        if (r < accuracyPercentage) {
            return new SingleScore(3 * treble, false);
        }
            ...
            return new SingleScore(treble, false);
        }
        else if (r < accuracyPercentage + 2 * (100 - accuracyPercentage)*0.2) {
            return new SingleScore(3 * DART_BOARD[0][treble], false);
        }
        else if (r < accuracyPercentage + 3 * (100 - accuracyPercentage)*0.2) {
            return new SingleScore(3 * DART_BOARD[1][treble], false);
        }
        else if (r < accuracyPercentage + 4 * (100 - accuracyPercentage)*0.2) {
            return new SingleScore(DART_BOARD[0][treble], false);
        }
        else {
            return new SingleScore(DART_BOARD[1][treble], false);
        }
    }

    public SingleScore throwDouble(int double_, int accuracyPercentage) {
        //  return result of throwing for double d with accuracy p%
        int r = ThreadLocalRandom.current().nextInt(100);

        if (r < accuracyPercentage) {
            return new SingleScore(2 * double_, true);
        }
            ...
            return new SingleScore(0, false);
        }
        else if (r < accuracyPercentage + 2 * (100 - accuracyPercentage)*0.3) {
            return new SingleScore(double_, false);
        }
            ...
            return new SingleScore(2 * DART_BOARD[0][double_], true);
        }
        else if (r < accuracyPercentage + 2 * (100 - accuracyPercentage)*0.3 + 3 * (100 - accuracyPercentage)*0.15) {
            return new SingleScore(2 * DART_BOARD[1][double_], true);
        }
            ...
            return new SingleScore(DART_BOARD[0][double_], false);
        }
        else {
            return new SingleScore(DART_BOARD[1][double_], false);
        }
    }
    ...
    public SingleScore throwSingle(int single, int accuracyPercentage, int accuracyOuterPercentage) {
        ...
        if (single == 25) {
            ...
            if (r < accuracyPercentage) {
                return new SingleScore(single, false);
            }
            else if (r < accuracyPercentage + 1 * (100 - accuracyPercentage)*0.3) {
                return new SingleScore(DART_BOARD[0][single], false);
            }
            else if (r < accuracyPercentage + 2 * (100 - accuracyPercentage)*0.3) {
                return new SingleScore(DART_BOARD[1][single], false);
            }
            else if (r < accuracyPercentage + 2 * (100 - accuracyPercentage)*0.3 + (100 - accuracyPercentage)*0.2) {
                return new SingleScore(3 * single, false);
            }
            else {
                return new SingleScore(2 * single, true);
            }
        }
    }
}
